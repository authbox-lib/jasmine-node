...
jasmine.Env.prototype.describe = function(description, specDefinitions) {
  ...
  suite.filename = jasmine.getEnv().currentFilename;
...
jasmine.ppWithNewline = function() {
  var rv = jasmine.pp.apply(this, arguments);
  if (rv.length > 64) {
    rv = '\n    ' + rv + '\n';
  }
  return rv;
  ...
  return function() {
    ...
    if (!result) {
      if (this.message) {
        ...
        message = "Expected " + jasmine.ppWithNewline(this.actual) + (this.isNot ? " not " : " ") + englishyPredicate;
        if (matcherArgs.length > 0) {
          for (var i = 0; i < matcherArgs.length; i++) {
            if (i > 0) message += ",";
            message += " " + jasmine.ppWithNewline(matcherArgs[i]);
          }
        }
        message += ".";
      }
...
jasmine.FakeTimer = function() {
  ...
  self.clearTimeout = function(timeoutObject) {
    self.scheduledFunctions[timeoutObject._id] = jasmine.undefined;
  };

  self.clearInterval = function(timeoutObject) {
    self.scheduledFunctions[timeoutObject._id] = jasmine.undefined;
  };
